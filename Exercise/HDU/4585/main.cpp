...
struct Fhq_Treap {
    ...
    int insert(T val, int id) {
        if (empty()) {
            root = New(val, id);
        } else {
            int x, y;
            split(root, x, y, val - 1);
            merge(x, x, New(val, id));
            merge(root, x, y);
        }
        return 0;
    }
...
int main() {
    ...
    while (scanf("%d", &n) != EOF && n) {
        ...
        for (int i = 0; i < n; i++) {
            ...
            val1 = treap.pre(val, id1);
            val2 = treap.suf(val, id2);
            treap.insert(val, id);
            int de1 = val - val1;
            int de2 = val2 - val;
            if (val1 == -1) {
